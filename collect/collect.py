...
COUNTY_RE = re.compile(
    r'((?:(?:St|Ste|Mt)\.\s+)?[A-Z][a-z]+(?:[ \-][A-Z][a-z]+)?)\s+(?:County|Parish)\b')
...
def score_article(item, topic, state, cfg, codebook):
    ...
    states = [s for s in cfg['states'] if s.lower() in text]
    counties = sorted({m.group(0) for m in COUNTY_RE.finditer(title)})
    if counties:
        score += 1
    if states:
        score += 1
